/*global window gadgets Mustache b$*/
...
define(["jquery",
        "jquery-ui",
        "launchpad/widgets/widget-catalog/slides.min"], function($) {

    "use strict";

    var URL_CATALOG = b$.portal.config.serverRoot +'/portals/'+ encodeURI(b$.portal.portalName) +'/catalog.xml?f=type(eq)WIDGET&ps=100';
     ...
     * @constructor
     */
    var WidgetCatalog = function(widget) {
    ...
    WidgetCatalog.prototype.init = function() {
        ...
        gadgets.pubsub.subscribe("containerReadyForCatalog", function(container) {
            self.currentContainer = container;
        });

        gadgets.pubsub.publish("widgetCatalogLoaded", self.widget);
    };
    ...
    WidgetCatalog.prototype.showPreview = function(itemId) {
        ...
        var preview = (item.preferences.previewUrl) ? item.preferences.previewUrl.value.replace('$(contextRoot)', b$.portal.config.serverRoot) : "";
